...
void main_receive_packet_loop(DMAcontext* dma_context, int thread_id) {
    ...
    while (1) {
        ...
        while(1) {
            ...
            if (time_span.count() > 10.0 && msgs_completed == 0 && dma_context->total_received > 0) {
        ...
        for(int msg=0; msg < msgs_completed; msg++) {
            ...
            if (!hash_table->isAlreadyDeclare[p4ml_header->agtr] && !isResend_packet)
                hash_table->isAlreadyDeclare[p4ml_header->agtr] = true;


            /* Check if Collision packet */
            bool is_collision_packet = p4ml_header->flag & 0x02;
            bool is_lzy_collision = p4ml_header->is_lzy_Col & 0x01;


  
            if(is_collision_packet || isResend_packet){
            ...
            if (receive_byte_reset_flag[thread_id]) {
                //receive_in_sec[thread_id] = 0;
                //receive_PS[thread_id] = 0;
               // receive_Aggr[thread_id] = 0;
               // receive_Total_Per_Task[thread_id] = 0;
...
void Start(int thread_id) {
    bindingCPU((appID-1)*MAX_THREAD_PER_APP+thread_id);
...
int main(int argc, char *argv[]) {
    ...
    max_agtr_size_per_thread = 50;
    UsedSwitchAGTRcount = 450;
    int tempargc = 2;
    ...
    if (argc > 2 ) {
        while (tempargc < argc){
            ...
            if (option == "-a") {
               ...
               max_agtr_size_per_thread = num_agtr;
            } 

            if (option == "-aa") {
                int num_used_agtr = atoi(argv[tempargc++]);
                UsedSwitchAGTRcount = num_used_agtr;
            }
            if (option == "-th") {
                int num_thread_argv = atoi(argv[tempargc++]);
                num_thread = num_thread_argv;
            }
    ...
    for (int i = 0; i < 2; i++){
        ib_dev = dev_list[i];
        if (strcmp(ibv_get_device_name(ib_dev),nic)==0){
            break;
        }
    }
    */
    ...
    while (1) {
        ...
        if (time_span.count() >= 0.5) {
            ...
            for (int i = 0; i < num_thread; i++) {
                ...
                total_bandwidth += receive_in_sec[i] * 194.0 / 1024.0 / 1024.0 / 1024.0 * 8.0 / time_span.count();
