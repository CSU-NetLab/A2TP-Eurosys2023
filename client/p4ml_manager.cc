...
int P4mlManager::nic = 0;
int P4mlManager::max_window_size = 0;
int P4mlManager::max_agtr_size_per_thread = 0;
int P4mlManager::UsedSwitchAGTRcount = 0;
int P4mlManager::_num_thread;
int P4mlManager::tail_time_range = 0;
int P4mlManager::pause_count = 0;
...
void P4mlManager::SetNic(int num)
{
    nic = num;
}
...
void P4mlManager::SetUsedSwitchAGTRcount(int used_agtr) 
{
 ...
 void P4mlManager::SetTailTimeRange(int trange){
     tail_time_range = trange;
     printf("\nSetTailTimeRange to 0~%d\n", trange);
 }

 void P4mlManager::SetMaxWindow(int size){
...
void P4mlManager::main_receive_packet_loop(DMAcontext* dma_context,
    int32_t* data,
    int my_id, CC_manager &cc_manager)
{
    unsigned seed = 171;
    srand(seed);
    int msgs_completed;
    ...
    this_pos_to_send = cc_manager.last_window;
    window = cc_manager.last_window;
    send_pointer = cc_manager.last_window;
    finish_window_seq = cc_manager.last_window;
    //cc_manager.aggr_count = max_agtr_size_per_thread;
    cc_manager.aggr_count = cc_manager.last_aggr_count;

    //cc_manager.cwnd_bytes = cc_manager.last_window * P4ML_PACKET_SIZE;


    //CC_manager cc_manager(max_agtr_size_per_thread, max_window_size);
    ...
    while (window_manager[my_id].last_ACK < window_manager[my_id].total_ACK) {
        ...
        for (int msg = 0; msg < msgs_completed; msg++) {
            ...
            bool is_lzy_Collision = p4ml_header->is_lzy_Col & 0x01; //check is_Col for A2TP
            ...
            if (!window_manager[my_id].isACKed[p4ml_header->seq_num] && !isOverflowRequest) {
                if(is_lzy_Collision){ // count number of Col for aggregator congestion control of A2TP
                    cc_manager.col_count++;
                }

                if (is_collision_packet && !is_lzy_Collision) {
                    collision_time++;
                    //printf("collision???\n");
                ...
                if (is_ecn_mark_packet){ // count number of ECN for link congestion control of A2TP
                    cc_manager.ecn_count++;
                }
                ...
                /* Update Window */
                int temp_ack_num = window_manager[my_id].UpdateWindow(&p4ml_header->seq_num);
                if(temp_ack_num) { //for calculating straggler degree
                    cc_manager.receive_ack += temp_ack_num;
                }                   
                ...
                if (next_seq_num <= total_packet && next_seq_num <= window_manager[my_id].last_ACK + window && next_seq_num > send_pointer) {
                    ...
                    for (int i = packet_to_process - 1; i >= 0; i--) {
                        ...
                        // set Terminated if last packet
                        //lzy
                        bool isForceCollision = false;
                        bool isResend = false;

                        if(cc_manager.aggr_count > 0){ //Determine whether to use an aggregator according to the size of the aggregator congestion window 
                            cc_manager.aggr_count--;
                            //isForceCollision = true;
                            isResend = false;
                        }else{
                            //isForceCollision = false;
                            isResend = true;
                        }
                        make_p4ml_layer_and_copy_to(send_region + (this_pos_to_send + to_be_sent) * P4ML_LAYER_SIZE, pushPulljobQueue[my_id].front(), app_info, &switch_agtr_pos, &process_next_seq_num, &next_offset, isResend, ForceForward, isOverflow[my_id][process_next_seq_num], isForceCollision);
                    ...
                    if (next_seq_num <= window_manager[my_id].last_ACK + window && next_seq_num > send_pointer) {
                        ...
                        if (next_seq_num <= total_packet) {
                            ...
                            for (int i = packet_to_process - 1; i >= 0; i--) {
                                ...
                                make_p4ml_layer_and_copy_to(send_region + (this_pos_to_send + to_be_sent) * P4ML_LAYER_SIZE, pushPulljobQueue[my_id].front(), app_info, &switch_agtr_pos, &process_next_seq_num, &next_offset, isResend, ForceForward, isOverflow[my_id][process_next_seq_num]);
                ...
                if (CC_ENABLE) {
                    ...
                    std::chrono::duration<double> time_span_lzy = std::chrono::duration_cast<std::chrono::duration<double>>(current_time - cc_manager.last_ts);
                    if (time_span_lzy.count() > baseRTT && cc_manager.update_urgent == 0 && newloopflag == 1 && time_span_lzy.count() < 0.0001){ //calculate straggler degree
                        cc_manager.urgent = 1.0*cc_manager.receive_ack/cc_manager.last_window;
                        cc_manager.urgent = cc_manager.urgent > 1 ? 1 : cc_manager.urgent;
                        cc_manager.update_urgent = 1;
                        //printf("receive: %d, lastwindow: %d, urgent %lf\n",cc_manager.receive_ack, cc_manager.last_window, cc_manager.urgent);
                    }
                    //printf("first col: %lf\n",time_span_lzy.count());
                    ...
                    if (p4ml_header->seq_num == finish_window_seq) {
                        ...
                        newloopflag = 1;
                        cc_manager.update_urgent = 0;
                        cc_manager.receive_ack = 0;
                        bool isECN = false;
                        bool isCol = false;
                        if (cc_manager.ecn_count > 0) isECN = true;
                        if (cc_manager.col_count > 0) isCol  = true;
                        isECN = is_ecn_mark_packet;
                        cc_manager.adjustWindow(isECN, isCol, appID, looptime);
                        cc_manager.urgent = 1;
                        int new_window = cc_manager.GetCwndPackets();
                        

                        cc_manager.aggr_count = cc_manager.GetAggrPackets();

                        if (send_pointer + new_window > window_manager[my_id].total_ACK)
                            window = window_manager[my_id].total_ACK - send_pointer;
                        else{
                            window = new_window;
                            cc_manager.last_window = window;
                            cc_manager.last_aggr_count = min(cc_manager.aggr_count, window);
                        }
                       
                        
                        if(window < cc_manager.aggr_count){
                            cc_manager.aggr_count = window;
                        }

                        finish_window_seq += window;

                        cc_manager.last_ts = std::chrono::high_resolution_clock::now();
                    }
        ...
        if(tail_time_range){ //useless
            pause_count += msgs_completed;
            if(pause_count > 4 * max_window_size){
                pause_count = 0;
                long long minvalue = 0;
                long long tailtime = tail_time_range + 1 + minvalue;
                //long long tailtime = tail_time_range;
                std::this_thread::sleep_for(std::chrono::nanoseconds(tailtime));
                //printf("tailtime: %lld\n",tailtime);
            }
...
void P4mlManager::init_threadPool(int num_thread)
{
    ...
    for (int i = 0; i < 2; i++){
        ...
        if (strcmp(ibv_get_device_name(ib_dev),"mlx5_1")==0){
    ...
    */
    //ib_dev = dev_list[0];
    //printf("list0: %s, list1: %s\n", ibv_get_device_name(dev_list[0]), ibv_get_device_name(dev_list[1]));
    if (!ib_dev) {
        fprintf(stderr, "IB device not found\n");
        exit(1);
    }
    printf("using: %s nic: %d\n", ibv_get_device_name(ib_dev), nic);
...
{
    bindingCPU(thread_id + nic * (appID-1) * MAX_THREAD_PER_APP); 
    ...
    while (1) {
        if (!pushPulljobQueue[thread_id].empty()) {
            ...
            //printf("num_first_time_sending:%d\n",num_first_time_sending);
            ...
            for (int j = 0; j < num_first_time_sending; j++) {
                ...
                //printf("send pkt no.%d\n",j);
            }

            // send_packet(dma_context, P4ML_LAYER_SIZE * num_first_time_sending, 0);

            main_receive_packet_loop(dma_context, data, my_id, cc_manager);
