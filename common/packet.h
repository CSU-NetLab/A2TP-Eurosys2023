...
#define P4ML_PACKET_SIZE 309 //308+1
#define P4ML_DATA_SIZE 248
#define P4ML_HEADER_SIZE 27 //26+1
#define P4ML_LAYER_SIZE 275 //274+1
...
#pragma pack(push, 1)
    struct agghdr {
        ...
        */
        ...
        uint16_t appID;
        uint16_t seq_num;
        uint8_t is_lzy_Col; //is_Col in A2TP
        uint16_t agtr;
        uint16_t agtr2;
...
void inline make_p4ml_layer_and_copy_to(void* payload, Job* job_info, AppInfo* app_info, uint16_t* agtr, uint16_t* seq_num, int* offset, bool isResend, bool isForceForward, bool isOverflow, bool isForceCollision = false)
{
    ...
    agg_header->is_lzy_Col = 0;
    //agg_header->flag = 0; //lzy
    if (isResend) 
        agg_header->flag |= 4;
    ...
    if (isForceCollision){ //lzy
        agg_header->flag |= 0x02;
    }
    // PS Index
...
void inline p4ml_header_resetLZYColBit(agghdr* p4ml_header){
    p4ml_header->is_lzy_Col = 0x00;

}
void inline p4ml_header_setLZYColBit(agghdr* p4ml_header){
    p4ml_header->is_lzy_Col = 0x01;
}
