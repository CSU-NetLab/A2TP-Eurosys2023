...
control ingress 
{
    ...
    if (valid(p4ml_entries)) { //aggregated packet

            if (ipv4.ecn == 3 or p4ml.ECN == 1) { //mdata.is_ecn=1
                apply(setup_ecn_table); //in common.p4
            }
            // ack packet
            if (p4ml.isACK == 1) {
                ...
                if (p4ml.overflow == 1 and p4ml.isResend == 0) {
                    ...
                    apply(clean_appID_and_seq_table); //in common.p4 and registers.p4 get mdata.isMyAppIDandMyCurrentSeq
                    ...
                    if (mdata.isMyAppIDandMyCurrentSeq != 0) {
                        ...
                        apply(clean_col_table); //used in A2TP
                ...
                if(ig_intr_md.resubmit_flag == 1) {
                    ...
                    apply(p4ml_resubmit); //resubmit mdata.agtr_time
                }
                
            } else {

                if (p4ml.overflow == 1) {
                    apply(outPort_table); //set eg according to appseq, ingress, dataindex, psindex in common.p4
                } else {
                    if (p4ml.isResend == 1) {
                        apply(appID_and_seq_resend_table); //clean appid and seq
                    } else {
                        apply(appID_and_seq_table); //allocate a new/existing block for current appid
                    }
                    // Correct ID and Seq
                    if (mdata.isMyAppIDandMyCurrentSeq != 0) {
                        ...
                        apply(col_register_table); //used in A2TP
                        
                        apply(bitmap_aggregate_table); //check aggregation bitmap get
                                                        //isaggregate? and bit_or
                        ...
                        if (p4ml.isResend == 1) {
                            ...
                            apply(agtr_time_table); //update agtr_time
                        }
 
                        // bitmap correct
                        if (mdata.isAggregate != 0) { //need to aggregate
                            if (mdata.current_agtr_time == p4ml.agtr_time) { // aggregation finish?
                                apply(noequ0_processEntry1andWriteToPacket); //sum to register
                                ...
                                apply(processEntry1); // cover or sum to register
                                apply(processEntry2);
                                apply(processEntry3);
                                apply(processEntry4);
                                apply(processEntry5);
                            ...
                            if (mdata.current_agtr_time == p4ml.agtr_time) {
                                apply(Entry1WriteToPacket); //write to p4ml_entries.data1; to packet
                        ...
                        if (p4ml.isResend == 0) {
                            ...
                            if(mdata.is_col == 1){
                                apply(tag_col_register_table); //used in A2TP
                            }
